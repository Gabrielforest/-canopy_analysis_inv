---
title: "Análise de dossel - Inventário Florestal"
format:
  html:
    toc: true
...
Parcela com maior quantidade de pixels na classe de gap grande (menor cobertura), foi a parcela `r dt[ which.max( dt[, 3 ] ), 1 ]` com `r max( dt[, 3 ] ) * 100`% dessa classe de espaçamento. 

Parcela com maior quantidade de pixels na classe de gap pequeno (maior cobertura de dossel) foi a parcela `r dt[ which.min( dt[, 4 ] ), 1 ]` com `r min( dt[, 4 ] ) * 100`% dessa classe de espaçamento.
...
colocar as fotos aqui e comentar o que foi observado.